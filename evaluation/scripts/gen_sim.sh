...
if [ $1 = 'ont' ]; then
	...
	rm *.fastq
	rm *.ref
	paftools.js pbsim2fq $DATA/GCA_000001405.15_GRCh38_no_alt_analysis_set.fasta.gz.fai *.maf > $DATA/ont_sim.fasta
	rm *.maf
elif [ $1 = 'hifi' ]; then
	...
	rm *.fastq
	rm *.ref
	paftools.js pbsim2fq $DATA/GCA_000001405.15_GRCh38_no_alt_analysis_set.fasta.gz.fai *.maf > $DATA/hifi_sim.fasta
	rm *.maf
