...
void encode_const_len(int fd, unsigned l, char *name) {
	...
	printf("Info: Binary file %s.bin written\n", name);
}
...
void encode_var_len(int fd, unsigned l, char *name) {
	...
	char *write_buf = (char *)malloc(l / 2 + l % 2 + 1);
	if (write_buf == NULL) {
		err(1, "malloc");
	}
	...
	while (i < size) {
		...
		size_t odd = 0;
		for (;;) {
			char first = seq_nt4_table[(size_t)read_buf[i]];
			i++;
			if (read_buf[i] == '\n') {
				write_buf[j] = first | (0xf << 4);
				odd          = 1;
				break;
			}
			write_buf[j] = first | (seq_nt4_table[(size_t)read_buf[i]] << 4);
			i++;
			j++;
			if (read_buf[i] == '\n') {
				write_buf[j] = 0xf;
