 ...
 *               where lastPos is the position of the last base of the i-th
 * minimizer, and strand indicates whether the minimizer comes from the top or
    ...
    for (i = l = buf_pos = min_pos = 0; i < len; ++i) {
                ...
                if (i + 1 < len && seq_nt4_table[(uint8_t)str[i + 1]] == c) {
                    for (skip_len = 2; i + skip_len < len; ++skip_len)
                        if (seq_nt4_table[(uint8_t)str[i + skip_len]] != c)
                            break;
                    i += skip_len - 1; // put $i at the end of the
                                       // current homopolymer run
                }
                tq_push(&tq, skip_len);
                kmer_span += skip_len;
                if (tq.count > k)
        ...
        if (l == w + k - 1 &&
            ...
            for (j = buf_pos + 1; j < w; ++j)
                if (min.x == buf[j].x && buf[j].y != min.y) {
                    val.minimizer = buf[j].x >> (8 + shift);
                    val.position = buf[j].y >> 1;
        ...
        if (info.x <= min.x) { // a new minimum; then write the old min
            if (l >= w + k && min.x != UINT64_MAX) {
                val.minimizer = min.x >> (8 + shift);
                val.position = min.y >> 1;
                val.strand = min.y & 1;
                kv_push(mm72_t, km, *p, val);
            }
