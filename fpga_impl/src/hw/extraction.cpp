...
void extract_minimizers(const base_t *read_i, min_stra_b_t *p_o, ap_uint<1> *p_en_o) {
...
LOOP_extract_minimizer:
	for (size_t i = 0; i < READ_LEN; ++i) {
		...
		if (c < 4) {                                                 // not an ambiguous base
			...
			if (l >= W + K - 1) {
				push_min_stra(p, p_o, p_en_o, p_l, min_reg);
				min_saved = 1;
			}
			l = 0;
		}
		...
		if (l == W + K - 1) {
			if (same_min) {
				push_min_stra(p, p_o, p_en_o, p_l, (min_stra_t){min_reg.minimizer, !min_reg.strand});
				same_min = 0;
			}
		}
...
void push_min_stra(min_stra_b_t *p, min_stra_b_t *p_o, ap_uint<1> *p_en_o, ap_uint<MIN_STRA_SIZE_LOG> &p_l,
                   min_stra_t val) {
