#include "extraction.h"
#include "kernel.h"
#include <stdio.h>
#include <string.h>
#define SHIFT1 (2 * (K - 1))
#define MASK ((1ULL << 2 * K) - 1)
#define MASK_B ((1ULL << B) - 1)

static inline uint64_t hash64(uint64_t key) {
	key = (~key + (key << 21)) & MASK; // key = (key << 21) - key - 1;
	key = key ^ key >> 24;
	key = ((key + (key << 3)) + (key << 8)) & MASK; // key * 265
	key = key ^ key >> 14;
	key = ((key + (key << 2)) + (key << 4)) & MASK; // key * 21
	key = key ^ key >> 28;
	key = (key + (key << 31)) & MASK;
	return key;
}

#ifdef VARIABLE_LEN
void extract_seeds(const char *read, seed_v *p, const size_t len) {
#else
void extract_seeds(const char *read, seed_v *p) {
#endif
	uint64_t kmer[2] = {0, 0};
	hash_t buff[256];
	unsigned int l = 0; // l counts the number of bases and is reset to 0 each
	                    // time there is an ambiguous base

	size_t buff_pos         = 0;
	unsigned int min_pos    = 0;
	hash_t min_reg          = {.hash = UINT64_MAX, .str = 0};
	unsigned char min_saved = 0;
	unsigned char same_min  = 0;

#ifdef VARIABLE_LEN
	for (size_t i = 0; i < len; i++) {
#else
	for (size_t i = 0; i < READ_LEN; ++i) {
#endif
		char c          = (i % 2) ? (read[i / 2] >> 4) : (read[i / 2] & 0xf);
		hash_t hash_reg = {.hash = UINT64_MAX, .str = 0};
		if (c < 4) {                                             // not an ambiguous base
			kmer[0] = (kmer[0] << 2 | c) & MASK;             // forward k-mer
			kmer[1] = (kmer[1] >> 2) | (3ULL ^ c) << SHIFT1; // reverse k-mer
			if (kmer[0] == kmer[1]) {
				continue; // skip "symmetric k-mers" as we don't
				          // know it strand
			}
			unsigned char z = kmer[0] > kmer[1]; // strand
			++l;
			if (l >= K) {
				hash_reg.hash = hash64(kmer[z]);
				hash_reg.str  = z;
			}
		} else {
			if (l >= W + K - 1) {
				push_seed(p, min_reg);
				min_saved = 1;
			}
			l = 0;
		}
		buff[buff_pos] = hash_reg;

		if (l == W + K - 1) {
			if (same_min) {
				push_seed(p, (hash_t){min_reg.hash, !min_reg.str});
				same_min = 0;
			}
		}

		if (hash_reg.hash <= min_reg.hash) {
			if (l >= W + K) {
				push_seed(p, min_reg);
			} else if (l < W + K - 1 && hash_reg.hash == min_reg.hash && hash_reg.str != min_reg.str) {
				same_min = 1;
			} else {
				same_min = 0;
			}
			min_reg   = hash_reg;
			min_pos   = buff_pos;
			min_saved = 0;
		} else if (buff_pos == min_pos) {
			if (l >= W + K - 1) {
				push_seed(p, min_reg);
			}
			min_reg.hash    = UINT64_MAX;
			char same_min_w = 0;
			for (size_t j = buff_pos + 1; j < W; j++) {
				if (min_reg.hash > buff[j].hash) {
					min_reg    = buff[j];
					min_pos    = j;
					min_saved  = 0;
					same_min_w = 0;
				} else if (min_reg.hash == buff[j].hash && min_reg.hash != UINT64_MAX) {
					min_pos = j;
					if (min_reg.str != buff[j].str) {
						same_min_w = 1;
					}
				}
			}
			for (size_t j = 0; j <= buff_pos; j++) {
				if (min_reg.hash > buff[j].hash) {
					min_pos    = j;
					min_reg    = buff[j];
					same_min_w = 0;
					min_saved  = 0;
				} else if (min_reg.hash == buff[j].hash && min_reg.hash != UINT64_MAX) {
					min_pos = j;
					if (min_reg.str != buff[j].str) {
						same_min_w = 1;
					}
				}
			}
			if (same_min_w && l >= W + K - 1) {
				push_seed(p, (hash_t){min_reg.hash, !min_reg.str});
			}
		}
		buff_pos = (buff_pos == W - 1) ? 0 : buff_pos + 1;
	}
	if (min_reg.hash != UINT64_MAX && !min_saved) {
		push_seed(p, min_reg);
	}
}

inline void push_seed(seed_v *p, const hash_t hash) {
	uint32_t seed = hash.hash & MASK_B;
	for (size_t i = 0; i < p->n; i++) {
		if (p->a[i].seed == seed && p->a[i].str == hash.str) {
			return;
		}
	}
	p->a[p->n].seed = seed;
	p->a[p->n].str  = hash.str;
	p->n++;
}
